...
public:
    ...
    ~CVideoStream() { if (m_hFile != INVALID_HANDLE_VALUE) CloseHandle(m_hFile); }
    ...
    STDMETHODIMP Seek(LARGE_INTEGER dlibMove, DWORD dwOrigin, ULARGE_INTEGER* plibNewPosition) {
        ...
        if (plibNewPosition) plibNewPosition->QuadPart = newPos.QuadPart - PADDING_SIZE;
        return S_OK;
    }
    STDMETHODIMP Stat(STATSTG* p, DWORD) {
        ZeroMemory(p, sizeof(STATSTG));
        p->type = STGTY_STREAM;
        p->cbSize.QuadPart = m_fileSize - PADDING_SIZE;
        return S_OK;
    }
...
public:
    ...
    HRESULT SetStream(IStream* pStream, LONGLONG length) {
        ...
        m_llLength = length; return S_OK;
    }

    // CBasePin already implements IUnknown via CUnknown — delegate explicitly
    STDMETHODIMP_(ULONG) AddRef()  { return CBasePin::AddRef(); }
    STDMETHODIMP_(ULONG) Release() { return CBasePin::Release(); }

    STDMETHODIMP QueryInterface(REFIID riid, void** ppv) {
        if (riid == IID_IAsyncReader) {
            *ppv = static_cast<IAsyncReader*>(this); AddRef(); return S_OK;
        }
    ...
    STDMETHODIMP SyncReadAligned(IMediaSample* pSample) override {
        ...
        BYTE* pBuf = NULL; pSample->GetPointer(&pBuf);
...
public:
    ...
    CBasePin* GetPin(int n)  override { return (n == 0) ? m_pPin : NULL; }
    ...
    HRESULT OpenFile(LPCWSTR pszFileName) {
        ...
        LARGE_INTEGER fs; GetFileSizeEx(hFile, &fs); m_fileSize = fs.QuadPart;
        CVideoStream* pStream = new CVideoStream(hFile, m_fileSize);
        m_pPin->SetStream(pStream, m_fileSize - PADDING_SIZE);
        pStream->Release();
...
// CVideoOutputPin ctor — after CVideoFilter is defined
CVideoOutputPin::CVideoOutputPin(CVideoFilter* pFilter, CCritSec* pLock, HRESULT* phr)
    : CBasePin(L"Output", pFilter, pLock, phr, L"Output", PINDIR_OUTPUT)
    , m_pStream(NULL), m_llLength(0) {}
